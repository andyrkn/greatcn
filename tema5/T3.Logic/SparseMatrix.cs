...
namespace T3.Logic
{
    public class SparseMatrix
    {
        ...
        public List<List<(double value, int column)>> Matrix { get; private set; }
        ...
        public static SparseMatrix RandomSparseSymmetricalMatrix(int size,int elementsPerLine)
        {
            ...
            for (int i = 0; i < res.Size; i++)
            {
                ...
                while (indices.Count < i && indices.Count < elementsPerLine) { indices.Add(r.Next(0, i)); }

                foreach (int j in indices)
                {
                    if (res.Matrix[i].Count < elementsPerLine && res.Matrix[j].Count < elementsPerLine)
                    {
                        var cellValue = r.NextDouble() * 5;
                        res.Matrix[i].Add((cellValue, j));
                        res.Matrix[j].Add((cellValue, i));
                    }
                    else
                    {
                        break;
        ...
        public void CheckSymmetricalMatrixRules()
        {
            for (int i = 0; i < Size && InvalidMatrix.valid; i++)
            {
                foreach (var cell in Matrix[i])
                {
                    if (Matrix[cell.column].Find(x => x.column == i).value != cell.value)
                    {
                        InvalidMatrix = (false, string.Format("Line {0} column {1} is not a symmetrical value {2} != {3}",
                            i, cell.column, Matrix[cell.column].Find(x => x.column == i).value, cell.value));
                        break;
                    }
                }
            }
